...
openp = []
filterdp = []
dest_ports = [i for i in range(22000,24000)]
def is_up(ip):
    icmp = IP(dst=ip)/ICMP()
...
def check_port(ip, port, result = 1):
    ...
    try:
        ...
        resp = sr1(p, timeout=0.1,verbose=False) # Sending packet
        if str(type(resp)) == "<type 'NoneType'>":
            result = 0
        elif resp.haslayer(TCP):
            if resp.getlayer(TCP).flags == 0x12:
                send_rst = sr(IP(dst=ip)/TCP(sport=src_port, dport=port, flags='AR'), timeout=1,verbose=False)
                result = 1
