...
def give_job():
    ...
    if not running_scan_flag:
        ...
        status = get_status(ip_port, scan_type)
        send_job_result(ip_port, status, report_id)
...
def send_job_result(ip_port, status, report_id):
    ...
    if not job_queue.empty():
        ...
        (ip_port, scan_type, report_id) = job_queue.get()
        # send_job_result(ip_port, get_status(ip_port, scan_type))
        send_job_result(ip_port, get_status(ip_port, scan_type), report_id)
    print("send job")
